...
// >>> AJUSTE AQUI: caminho correto da sua tela de login.
...
class _InscricoesScreenState extends State<InscricoesScreen> {
  ...
  // Evita que o timer e o pull-to-refresh se sobreponham.
  ...
  void initState() {
    ...
    pesquisaController.addListener(filtrar);

    timerAtualizacao = Timer.periodic(
      const Duration(minutes: 1),
      // Refresh silencioso: não pisca a tela.
  ...
  Future<void> carregar({bool silencioso = false}) async {
    ...
    // Trava: não permite carregamentos simultâneos.
    ...
    try {
      ...
      // =================================================
      // SESSÃO MORTA → VOLTA PARA O LOGIN
      // =================================================
      //
      // O buscarInscricoesPDF já apagou a sessão local.
      // Então não dá para continuar nesta tela.
    ...
    setState(() {
      filtrados = dados.where((e) {
        ...
        return nome.contains(texto) ||
            ...
            data.contains(texto) ||
            turno.contains(texto);
      }).toList();
    });
  }

  Future<void> atualizarEInformar() async {
    await carregar();
